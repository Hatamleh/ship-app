...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<div class="min-h-screen bg-background flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8"><div class="max-w-md w-full space-y-8"><div><span class="mx-auto block w-14 text-primary">`);
		QacartMark($$renderer, {});
		$$renderer.push(`<!----></span> <h1 class="mt-6 text-center text-3xl font-extrabold text-foreground font-display">${escape_html(t("auth.loginTitle"))}</h1></div> <form class="mt-8 space-y-6">`);
		$$renderer.push("<!--[-1-->");
